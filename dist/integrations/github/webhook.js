...
export function verifyGitHubSignature(rawBody, signatureHeader, secret, fixtureMode = false) {
    if (!secret)
        return fixtureMode;
...
export function ingestPushPayload(payload, options = {}) {
    ...
    for (const commit of commits) {
        for (const file of filesForCommit(commit)) {
            ...
            if (!content || wasRemoved(commit, file.filename))
                continue;
...
function wasRemoved(commit, filename) {
    return (commit.removed ?? []).includes(filename);
}
function toIncident(finding) {
...
function rotationChecklistFor(provider) {
