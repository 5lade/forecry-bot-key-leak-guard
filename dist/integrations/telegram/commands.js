...
${repoGroups}`;
        ...
        case '/workspaces': {
            ...
            if (subcommand === 'create')
                return createClientWorkspace(identity, parts.slice(2).join(' ')).text;
            if (subcommand === 'list')
                return listClientWorkspaces(identity).text;
            if (subcommand === 'select')
                return selectClientWorkspace(identity, parts.slice(2).join(' ')).text;
