...
export function routeSettingsCommand(context) {
    ...
    try {
        switch ((subcommand ?? 'show').toLowerCase()) {
            ...
            case 'severity': {
                const value = rest[0];
                if (!value)
                    return 'Usage: /settings severity <low|medium|high|critical>';
                const policy = updateAlertPolicy(context.workspaceId, { severityThreshold: value });
            ...
            case 'digest': {
                const value = rest[0];
                if (!value)
                    return 'Usage: /settings digest <HH:MM UTC>';
            ...
            case 'reminder': {
                const value = rest[0];
                if (!value)
                    return 'Usage: /settings reminders <5-240m>';
            ...
            case 'repo': {
                ...
                const repo = rest[1];
                if ((action !== 'enable' && action !== 'disable') || !repo)
                    return 'Usage: /settings repo <enable|disable> <owner/name>';
            ...
            case 'spend-threshold': {
                ...
                if (!provider || !multiplier)
                    return 'Usage: /settings spend <provider> <multiplier>, for example /settings spend openai 3x';
...
export function renderSettings(workspaceId) {
