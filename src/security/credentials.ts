...
export function encryptCredentials(credentials: unknown, secret: string): Buffer {
...
export function decryptCredentials<T = unknown>(encrypted: Buffer | Uint8Array | string, secret: string): T {
  ...
  if (payload?.v !== VERSION || !payload.iv || !payload.tag || !payload.data) throw new Error('invalid_encrypted_credentials');
...
export function encryptedCredentialPreview(encrypted: Buffer | Uint8Array | string): string {
...
function keyFromSecret(secret: string): Buffer {
  if (!secret || secret.length < 16) throw new Error('credential_encryption_secret_too_short');
  return createHash('sha256').update(`key-leak-guard-credentials:${secret}`).digest();
}

function redactUnsafePrototype(value: unknown): unknown {
