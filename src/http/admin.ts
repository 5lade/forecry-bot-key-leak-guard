...
export function registerAdminRoutes(app: any, config: AppConfig) {
  ...
  app.get('/admin', async (request: any, reply: any) => {
    ...
    if (!snapshot) return reply.code(404).type('text/html; charset=utf-8').send(page('Workspace not found', '<p>Workspace not found. Send <code>/start</code> in Telegram first.</p>'));
...
export function createAdminToken(config: Pick<AppConfig, 'hmacSecret' | 'nodeEnv'>, workspaceId: string): string {
  return createHmac('sha256', adminSecret(config)).update(`admin:v1:${workspaceId}`).digest('base64url');
}

function isValidAdminToken(config: AppConfig, workspaceId: string, token?: string): boolean {
...
function renderAdminPage(config: AppConfig, snapshot: NonNullable<ReturnType<typeof snapshotForWorkspace>>): string {
...
function adminSecret(config: Pick<AppConfig, 'hmacSecret' | 'nodeEnv'>): string {
  if (config.hmacSecret) return config.hmacSecret;
  if (config.nodeEnv === 'production') throw new Error('HMAC_SECRET required for admin tokens');
