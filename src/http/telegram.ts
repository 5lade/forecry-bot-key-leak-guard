...
export function registerTelegramRoutes(app: any, config: AppConfig) {
  app.post('/webhooks/telegram', async (request: any, reply: any) => {
    if (config.telegramWebhookSecret) {
      ...
      if (provided !== config.telegramWebhookSecret) return reply.code(401).send({ ok: false, error: 'invalid_telegram_secret' });
    }
    ...
    if (update.message?.text) return reply.code(200).send({ ok: true, method: 'sendMessage', chat_id: update.message.chat.id, text: routeTelegramCommand(update.message) });
