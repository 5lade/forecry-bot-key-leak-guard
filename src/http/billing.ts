...
export function registerBillingRoutes(app: any, config: AppConfig) {
  app.post('/billing/checkout', async (request: any, reply: any) => {
    const body = parsedBodyFrom(request.body) as Record<string, unknown>;
    const workspaceId = stringParam(body.workspace_id ?? body.workspaceId);
    const plan = normalizePlan(body.plan);
    if (!workspaceId || !getWorkspace(workspaceId)) return reply.code(404).send({ ok: false, error: 'workspace_not_found' });
    if (!plan || plan === 'trial') return reply.code(400).send({ ok: false, error: 'paid_plan_required' });
  ...
  app.get('/billing/portal', async (request: any, reply: any) => {
  ...
  app.post('/billing/stripe/webhook', async (request: any, reply: any) => {
    ...
    if (!verifyStripeSignature(rawBody, signature, config.stripeWebhookSecret, fixtureMode)) return reply.code(401).send({ ok: false, error: 'invalid_stripe_signature' });
    ...
    if (parsed.ignored) return reply.code(202).send(parsed);
