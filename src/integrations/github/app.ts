...
export function githubSetupUrl(config: AppConfig, workspaceId: string): string {
...
export function githubCredentialStatus(config: AppConfig): { ok: boolean; missing: string[]; message?: string } {
  const missing: string[] = [];
  if (!config.githubAppId) missing.push('GITHUB_APP_ID');
  if (!config.githubAppPrivateKey) missing.push('GITHUB_APP_PRIVATE_KEY');
  if (!config.appBaseUrl) missing.push('APP_BASE_URL');
  if (!missing.length || config.localFixtureMode || config.nodeEnv !== 'production') return { ok: true, missing };
...
export function syncFixtureInstallation(input: { workspaceId: string; installationId?: number | string | bigint; repositories?: RepoSelection[] }) {
