import type { RepositoryRecord, WorkspaceRecord } from '../domain/types.js';
import { ensureTelegramWorkspace, listSnapshotsForAccount, snapshotForTelegramChat, setActiveWorkspaceForTelegramChat, type OnboardingSnapshot, type TelegramIdentity } from '../onboarding/localStore.js';
import type { LocalIncidentRecord } from '../integrations/telegram/store.js';

const repoClients = new Map<string, string>();
...
export function createClientWorkspace(identity: TelegramIdentity, name: string): WorkspaceCommandResult {
  const base = ensureTelegramWorkspace(identity);
  const clientName = normalizeClientName(name);
  if (!clientName) return { ok: false, text: 'Usage: /workspace create <client name>' };
...
export function listClientWorkspaces(identity: TelegramIdentity): WorkspaceCommandResult {
...
export function selectClientWorkspace(identity: TelegramIdentity, selector: string): WorkspaceCommandResult {
  const active = ensureTelegramWorkspace(identity);
  const needle = selector.trim().toLowerCase();
  if (!needle) return { ok: false, text: 'Usage: /workspace select <workspace id|name>' };
  ...
  if (!match) return { ok: false, text: `Unknown workspace "${selector}". Use /workspace list to see available client workspaces.` };
...
export function assignRepositoryToClient(snapshot: OnboardingSnapshot, repoSelector: string, clientNameInput: string): WorkspaceCommandResult {
  const repo = findRepo(snapshot.repositories, repoSelector);
  const clientName = normalizeClientName(clientNameInput);
  if (!repo || !clientName) return { ok: false, text: 'Usage: /workspace assign <owner/repo> <client name>' };
...
export function renderRepositoryGroups(repositories: RepositoryRecord[]): string {
  if (!repositories.length) return 'Repos by client: none connected yet.';
...
export function renderClientDigestSections(snapshot: OnboardingSnapshot, incidents: LocalIncidentRecord[]): string {
  const groups = groupRepositoriesByClient(snapshot.repositories);
  if (!Object.keys(groups).length) return 'Client digest sections: none — connect repositories first.';
...
function groupRepositoriesByClient(repositories: RepositoryRecord[]): Record<string, RepositoryRecord[]> {
  const groups: Record<string, RepositoryRecord[]> = {};
...
function findRepo(repositories: RepositoryRecord[], selector: string): RepositoryRecord | undefined {
  const needle = selector.trim().toLowerCase();
