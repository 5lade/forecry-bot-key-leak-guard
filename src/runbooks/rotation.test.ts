...
test('supported providers have complete rotation runbooks', () => {
  for (const provider of requiredProviders) {
    assert.ok(SUPPORTED_ROTATION_PROVIDERS.includes(provider as any), `${provider} should be supported`);
    ...
    for (const phrase of requiredPhrases) assert.match(checklist.join('\n'), phrase, `${provider} missing ${phrase}`);
  }
});
