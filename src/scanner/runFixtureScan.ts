...
for (const fixture of fixtureCases) {
  ...
  if (fixture.expectedFindings > 0 && found) truePositives += 1;
  if (fixture.expectedFindings > 0 && !found) falseNegatives += 1;
  if (fixture.expectedFindings === 0 && found) falsePositives += 1;
  if (fixture.expectedFindings === 0 && !found) trueNegatives += 1;
  const serialized = JSON.stringify(result.findings);
  findingPayloads.push(serialized);
  if (fixture.rawSecret && serialized.includes(fixture.rawSecret)) leakedRawSecret = true;
}
