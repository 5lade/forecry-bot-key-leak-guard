...
import type { ScanInput, ScanSummary, SecretFinding, SecretProvider } from './types.js';
...
export function scanText(input: ScanInput): ScanSummary {
  ...
  for (const detector of DETECTORS) {
    ...
    for (const match of input.content.matchAll(regex)) {
      ...
      if (isAllowed(raw, input.content, index)) continue;
...
export function fingerprintSecret(secret: string, provider: SecretProvider, hmacSecret: string): string {
...
function redactExcerpt(content: string, index: number, raw: string): string {
...
function isAllowed(raw: string, content: string, index: number): boolean {
  const normalized = raw.replace(/[^A-Za-z0-9]/g, '');
  if (LOW_VALUE.test(normalized)) return true;
  const context = content.slice(Math.max(0, index - 96), Math.min(content.length, index + raw.length + 96));
  if (SAFE_CONTEXT.test(context)) return true;
  return false;
}

function scoreConfidence(base: number, raw: string, content: string, index: number, provider: SecretProvider): number {
  const context = content.slice(Math.max(0, index - 96), Math.min(content.length, index + raw.length + 96));
  let score = base;
  if (CONTEXT_BOOST.test(context)) score += 0.04;
  if (provider === 'generic') {
    score += entropyBitsPerChar(raw) > 4.2 ? 0.16 : -0.18;
    if (raw.length >= 48) score += 0.05;
  }
  if (/['"]/.test(context)) score += 0.01;
...
function severityFor(base: Severity, confidence: number): Severity {
  if (confidence >= 0.93) return base === 'medium' ? 'high' : base;
  if (confidence >= 0.84) return base;
...
function locate(starts: number[], index: number): { lineNumber: number; columnStart: number } {
  let lo = 0;
