...
function trueCase(id: string, provider: string, label: string, rawSecret: string): FixtureCase {
  return { id, provider, content: `${label}=${JSON.stringify(rawSecret)}`, rawSecret, expectedFindings: 1 };
}

const trueSecrets: FixtureCase[] = [
...
function safeLookalike(i: number): string {
  const templates = [
