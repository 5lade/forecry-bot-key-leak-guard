...
import type { GitHubIncidentPayload, GitHubWebhookFinding } from '../integrations/github/types.js';
...
export interface RepoScanSource {
  repo: string;
  defaultBranch?: string;
  files: RepoScanFile[];
}

export interface RepoScanCheckpoint {
  repo: string;
  scannedFiles: number;
  totalFiles: number;
  findings: number;
...
export interface RepoScanResult {
  ...
  durationMs: number;
  checkpoint: RepoScanCheckpoint;
}

const ZERO_SEVERITIES: Record<Severity, number> = { low: 0, medium: 0, high: 0, critical: 0 };

const FIXTURE_SOURCES: RepoScanSource[] = [
  {
...
export function fixtureRepoSources(): RepoScanSource[] {
  return FIXTURE_SOURCES.map((source) => ({ ...source, files: source.files.map((file) => ({ ...file })) }));
}

export function sourceForRepo(repo: string, sources: RepoScanSource[] = FIXTURE_SOURCES): RepoScanSource | undefined {
  return sources.find((source) => source.repo.toLowerCase() === repo.toLowerCase());
}

export function scanRepositorySource(source: RepoScanSource, options: { hmacSecret?: string; now?: Date } = {}): RepoScanResult {
  ...
  const findingsBySeverity = findings.reduce<Record<Severity, number>>((counts, finding) => {
...
function manualFindingToIncident(finding: GitHubWebhookFinding): GitHubIncidentPayload {
